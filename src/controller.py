...
class PIDController:
    ...
    def compute(self, error, current_time=None):
        ...
        # clamp dt to avoid huge jumps on first frame or after pauses
        if dt <= 0.0 or dt > 1.0:
            dt = 0.02

        P = self.Kp * error

        # anti-windup: don't accumulate integral when saturated,
        # unless error is pushing the output back in range
        ...
        # smooth the derivative to reduce noise spikes
