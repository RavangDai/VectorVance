"""
safety.py - Simulated obstacle detection for testing without hardware.

Estimates distances by mapping frame brightness to distance values.
Not accurate - purely for development/sim use. Replace with HardwareObstacleDetector
...
class ObstacleDetector:
    ...
    def simulate_sensors(self, frame, steering_error):
        """
        Fake sensor readings derived from frame brightness.
        Brighter region = farther away (no obstacle).
        """
        h, w = frame.shape[:2]
        ...
        self.sensors['front']['distance']       = max(5, min(self.max_range, (front_brightness / 255) * self.max_range + noise()))
        self.sensors['front_left']['distance']  = max(5, min(self.max_range, (left_brightness  / 255) * self.max_range + noise()))
        self.sensors['front_right']['distance'] = max(5, min(self.max_range, (right_brightness / 255) * self.max_range + noise()))
